#!/bin/bash
# Full pi0.5 LoRA v2 run, 30,000-step budget. 1 GPU. Measured 1.636-1.64
# s/step (jobs 3872346 / 3937047) => ~13.6 h. Request well under 24 h.
#
# WALLTIME RAISED AT LAUNCH (2026-08-31), from 16:00:00 to 20:00:00. The original
...
# forward-only batch that is ~4.7 min per eval, x 30 evals = ~2.4 h on top of
# 13.7 h of training. The old request left no margin at all (13.7 + 2.4 = 16.0 h). Requested walltime is
...
make train-slurm \
    CONFIG=configs/realdata/pi05_lora_30k/train.yaml \
    DATA="$DATA" \
    TIME=20:00:00 CPUS=16 MEM=128G \
    EXTRA="$EPISODES"
